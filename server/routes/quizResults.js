...
router.post('/submit', auth, [
  ...
  body('answers.*.selectedAnswer').isIn(['a', 'b', 'c', 'd']).withMessage('Valid answer is required')
], async (req, res) => {
  try {
    ...
    // Update student's average score and total attempts
...
router.get('/my-results', auth, async (req, res) => {
  try {
    ...
    const { subject, chapter, limit = 50, sort = '-dateAttempted' } = req.query;
...
router.get('/leaderboard', async (req, res) => {
  try {
    ...
    // Aggregate quiz results to calculate averages
...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    // Check permissions: Students can only view their own results
